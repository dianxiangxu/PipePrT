...
import pipeprt.gui.undo.ChangeRateParameterEdit;
import pipeprt.gui.undo.ClearRateParameterEdit;
import pipeprt.gui.undo.GroupTransitionEdit;
import pipeprt.gui.undo.SetRateParameterEdit;
import pipeprt.gui.undo.TransitionPriorityEdit;
import pipeprt.gui.undo.TransitionRateEdit;
import pipeprt.gui.undo.TransitionRotationEdit;
import pipeprt.gui.undo.TransitionServerSemanticEdit;
import pipeprt.gui.undo.TransitionTimingEdit;
import pipeprt.gui.undo.UndoableEdit;
import pipeprt.gui.widgets.EscapableDialog;
import pipeprt.gui.widgets.TransitionEditorPanel;
 ...
 *         delay. to the
 */
public class PipeTransition extends PlaceTransitionObject {
	...
	public String getSubnetFile(){
		return subnetFile;
	}
	
	public void setSubnetFile(String subnetFile){
		this.subnetFile = subnetFile;
	}

	public File getSubnetFileHandler(){
		File subnet = new File(subnetFile);
		if (subnet.getName().equals(subnetFile) && CreateGui.getPrTPanel()!=null && CreateGui.getPrTPanel().getFile()!=null)
			subnet = new File(CreateGui.getPrTPanel().getFile().getParent()+File.separator+subnetFile);
		return subnet;
	}
	
	public boolean hasValidSubnetFile(){
		return isValidSubnetFile(subnetFile);
	}
	
	public static boolean isValidSubnetFile(String fileName){
		if (fileName==null || fileName.trim().equals(""))
			return false;
		if (!fileName.toLowerCase().endsWith("xml"))
			return false;
		File subnet = new File(fileName);
		return subnet!=null;
	}

	public boolean subnetFileExists(){
		if (!hasValidSubnetFile())
			return false;
		File subnetFileHandler = getSubnetFileHandler();
		return subnetFileHandler!=null && subnetFileHandler.exists();
	}
	 ...
	 *            TODO
	 */
	PipeTransition(double positionXInput, double positionYInput,
	...
	public PipeTransition paste(double x, double y, boolean fromAnotherView,
			DataLayerInterface model) {
		PipeTransition copy = TransitionFactory.createTransition(Grid.getModifiedX(x + this.getX()
		...
		copy.transition.transform(AffineTransform.getRotateInstance(Math
				.toRadians(copy.angle), PipeTransition.TRANSITION_HEIGHT / 2,
				PipeTransition.TRANSITION_HEIGHT / 2));
		copy.rateParameter = null;// this.rateParameter;
		
		copy.guard = this.guard;
		copy.effect = this.effect;
		copy.subnetFile = "";
		return copy;
	}

	public PipeTransition copy() {
		PipeTransition copy = TransitionFactory.createTransition(ZoomController.getUnzoomedValue(this.getX(),
...
//			g2.setPaint(Constants.ENABLED_TRANSITION_COLOUR);
		...
		g2.draw(transition);
		if (hasValidSubnetFile() && !highlighted){
			if (subnetFileExists())
...
*/	}
	...
	class ArcAngleCompare implements Comparable {
		...
		private Arc arc;
		private PipeTransition transition;
		private double angle;

		public ArcAngleCompare(Arc _arc, PipeTransition _transition) {
			arc = _arc;
	...
	private ArrayList<PipeTransition> groupTransitionsValidation(){
		if (!this.isSelected()) {
		...
		ArrayList<PipeTransition> transitionsToHide = new ArrayList<PipeTransition>();
			...
			if (pnObjects.get(i).isSelected()) {
				...
				if (pnObjects.get(i) instanceof PipeTransition) {
					...
					if (this != ((PipeTransition) pnObjects.get(i))) {
						...
						currentInputPlaces = new ArrayList<PipePlace>();
						Iterator arcInputIterator = ((PipeTransition) pnObjects
								.get(i)).getArcsTo();
						while (arcInputIterator.hasNext()) {
							tempArc = (Arc)arcInputIterator.next();
							currentInputPlaces.add((PipePlace) (tempArc.getSource()));
						}
					...
					transitionsToHide.add(((PipeTransition) pnObjects
							.get(i)));
				}
			}
		}
	...
	public UndoableEdit groupTransitions() {
		...
		ArrayList<PipeTransition> transitionsToHide = groupTransitionsValidation();
	...
	public void groupTransitionsHelper(ArrayList<PipeTransition> transitionsToHide, 
		...
		int i = 0;
		for (PipeTransition transitionToGroup:transitionsToHide) {
		...
		while (arcsTo.hasNext()) {
			...
			Arc newArc = ArcFactory.createNormalArc(tempArc.getStartPositionX(), tempArc.getStartPositionY(), tempArc.getArcPath().getPoint(1).getX(),
					tempArc.getArcPath().getPoint(1).getY(), tempArc.getSource(), newGroupTransition, new LinkedList<PipeMarking>(), "", false);
		...
		while (arcsFrom.hasNext()) {
			...
			Arc newArc = ArcFactory.createNormalArc(tempArc.getStartPositionX(), tempArc.getStartPositionY(), tempArc.getArcPath().getPoint(1).getX(),
					tempArc.getArcPath().getPoint(1).getY(), newGroupTransition, tempArc.getTarget(), new LinkedList<PipeMarking>(), "", false);
