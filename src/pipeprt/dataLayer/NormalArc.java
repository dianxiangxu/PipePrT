...
import pipeprt.gui.undo.ClearInverseArcEdit;
import pipeprt.gui.undo.JoinInverseArcEdit;
import pipeprt.gui.undo.SetInverseArcEdit;
import pipeprt.gui.undo.SplitInverseArcEdit;
import pipeprt.gui.undo.TagArcEdit;
...
public class NormalArc extends Arc {
	...
	NormalArc(NormalArc arc) {
		...
		this.setWeight((LinkedList<PipeMarking>) ObjectDeepCopier.mediumCopy(arc
				.getWeight()));
	...
	private void updateArc(boolean isJoined) {
		...
		if (isJoined) {
			...
			PipeTransition transition = this.getTransition();
