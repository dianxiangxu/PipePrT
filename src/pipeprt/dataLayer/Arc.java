...
import pipeprt.gui.undo.AddArcPathPointEdit;
import pipeprt.gui.undo.ArcWeightEdit;
import pipeprt.gui.undo.UndoableEdit;
import pipeprt.gui.widgets.ArcWeightEditorPanel;
...
public abstract class Arc extends PetriNetObject implements Cloneable {
	...
	/** Current Marking */
	protected LinkedList<PipeMarking> weight = new LinkedList<PipeMarking>();
	...
	public Arc(double startPositionXInput, double startPositionYInput,
		...
		setWeight((LinkedList<PipeMarking>) ObjectDeepCopier.mediumCopy(weightInput));
...
//8/04/2011 D. XU
//	setWeight seems to be incorrect for InhibitorArc.
//	It should be overridden in not only NormalArc but also InhibitorArc
	/**
	 * Set weight
	 * 
	 * @param weightInput
	...
	public void removeLabelsFromArc(){
			...
			for (NameLabel label : weightLabel) {
				...
				// D. Xu, for inhibitor and bidirectional arcs, getParent() is null
				if (getParent()!=null)				
	 ...
	 */
	public LinkedList<PipeMarking> getWeight() {
		return weight;
	}
...
//		}
	...
	public PipeTransition getTransition() {
		if (getTarget() instanceof PipeTransition) {
			return (PipeTransition) getTarget();
		} else {
			return (PipeTransition) getSource();
		}
	}
	...
	public String getWeightOfTokenClass(String id) {
		if (weight != null) {
			for (PipeMarking m : weight) {
