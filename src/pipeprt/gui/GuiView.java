...
import pipeprt.dataLayer.PlaceFactory;
import pipeprt.dataLayer.PlaceTransitionObject;
import pipeprt.dataLayer.RateParameter;
import pipeprt.dataLayer.TransitionFactory;
import pipeprt.gui.handler.AnnotationNoteHandler;
import pipeprt.gui.handler.ArcHandler;
import pipeprt.gui.handler.GroupTransitionHandler;
import pipeprt.gui.handler.LabelHandler;
import pipeprt.gui.handler.ParameterHandler;
import pipeprt.gui.handler.PlaceHandler;
import pipeprt.gui.handler.TransitionHandler;
...
//   private boolean animationmode = false;
   ...
   public GuiView(DataLayerInterface _model, PrTPanel _app) {
      model = _model;
      app = _app;
   ...
   public void addNewPetriNetObject(PetriNetObject newObject) {
      if (newObject != null) {
         if (newObject.getMouseListeners().length == 0) {
            if (newObject instanceof PipePlace) {
               LabelHandler labelHandler =
                       new LabelHandler(((PipePlace)newObject).getNameLabel(),
                       (PipePlace)newObject);
               ((PipePlace)newObject).getNameLabel().addMouseListener(labelHandler);
               ((PipePlace)newObject).getNameLabel().addMouseMotionListener(labelHandler);
               ((PipePlace)newObject).getNameLabel().addMouseWheelListener(labelHandler);
               
               PlaceHandler placeHandler =
                       new PlaceHandler(this, (PipePlace)newObject);
               ...
               add(newObject);
            } else if (newObject instanceof PipeTransition) {
               TransitionHandler transitionHandler =
                       new TransitionHandler(this, (PipeTransition)newObject);
               LabelHandler labelHandler =
                       new LabelHandler(((PipeTransition)newObject).getNameLabel(),
                       (PipeTransition)newObject);
               ((PipeTransition)newObject).getNameLabel().addMouseListener(labelHandler);
               ((PipeTransition)newObject).getNameLabel().addMouseMotionListener(labelHandler);
               ((PipeTransition)newObject).getNameLabel().addMouseWheelListener(labelHandler);
...
*/   
   ...
   class MouseHandler extends MouseInputAdapter {
      ...
      private PlaceTransitionObject newTransition(Point p, boolean timed){
         ...
         ((PipeTransition)pnObject).setTimed(timed);
         model.addPetriNetObject(pnObject);
         view.addNewPetriNetObject(pnObject);
         return (PlaceTransitionObject)pnObject;
      }
         ...
         if (SwingUtilities.isLeftMouseButton(e)) {
            ...
            switch (mode){
               ...
               case Constants.TRANSITION:
                  boolean timed = (mode == Constants.TRANSITION ? true : false);
               ...
               case Constants.FAST_TRANSITION:
                  if (e.isMetaDown() || metaDown) { // provisional
                     ...
                     view.newPNO = true;
                     timed = e.isAltDown();
                     if (app.getOldMode() == Constants.TRANSITION){
                        timed = !timed;
                     } 
