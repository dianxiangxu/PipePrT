...
public class CopyPasteManager 
        ...
        implements pipeprt.gui.Zoomable, java.awt.event.MouseListener, 
   ...
   public void showPasteRectangle(GuiView view) {
      if (!pasteInProgress) {
         ...
         try {
            ...
            // the following statement produces NullPointerException: view.getMousePosition()==null
...
//            npe.printStackTrace();
   ...
   private void doPaste(GuiView view){
      ...
      for (int i = 0; i < ptaToPaste.size(); i++) {
         ...
         if (pnObject != null) {
            if ((pnObject instanceof PipeTransition) && (sourceView != view)) {
               RateParameter rateParameter = 
                       ((PipeTransition)pnObject).getRateParameter();
               if (rateParameter != null) {
                  ((PipeTransition)pnObject).clearRateParameter();
               }
            } 
